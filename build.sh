...
# 编译 patch
curl -fLO https://ftp.gnu.org/gnu/patch/patch-2.8.tar.gz
tar -zxf patch-2.8.tar.gz
cd patch-2.8
./configure --prefix=/opt/patch-2.8-ohos-arm64
make -j$(nproc)
make install
cd ..

# 进行代码签名
cd /opt/patch-2.8-ohos-arm64
...
cp patch-2.8/COPYING /opt/patch-2.8-ohos-arm64/
cp patch-2.8/AUTHORS /opt/patch-2.8-ohos-arm64/

# 打包最终产物
cp -r /opt/patch-2.8-ohos-arm64 ./
tar -zcf patch-2.8-ohos-arm64.tar.gz patch-2.8-ohos-arm64
